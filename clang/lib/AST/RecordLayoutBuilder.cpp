...
>>>>>>> 0bcf37dab (fix(clang-cl): added missing padded struct size warning)
...
void MicrosoftRecordLayoutBuilder::computeVtorDispSet(
  ...
  // possible for a partially constructed object with virtual base overrides
  // to escape a non-trivial constructor.
  assert(RD->getMSVtorDispMode() == MSVtorDispMode::ForVBaseOverride);
  // Compute a set of base classes which define methods we override.  A
  // virtual base in this set will require a vtordisp.  A virtual base that
  // transitively contains one of these bases as a non-virtual base will also
  // require a vtordisp.
